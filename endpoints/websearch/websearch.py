...
class WebSearchInput(BaseModel):
    ...
    region: str | None = Field(
        default="",
        description="Two-letter region code (ISO 3166-1), e.g. 'US'",
        pattern=r'^[A-Za-z]{2}$'
    )
    safe: bool = Field(
        default=False,
        description="Enable SearXNG safesearch (filter explicit results)"
    )
...
async def greenfish_websearch(params: WebSearchInput) -> str:
    ...
    locale = (
        f"{params.lang}-{params.region.upper()}"
        if params.region
        else params.lang
    )
    ...
    try:
        ...
        return (
            f"Error: SearXNG instance returned "
